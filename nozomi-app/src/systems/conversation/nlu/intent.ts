...
const FAREWELL =
  /\b(bye|goodbye|see you|see ya|good night|goodnight|cya)\b|^(later|not now)$/i
const FAREWELL_JP = /^(さようなら|またね|じゃあね|バイバイ|おやすみ)/
const HELP =
...
export function detectIntent(input: string): Intent {
  ...
  if (FAREWELL.test(t) || FAREWELL_JP.test(t)) return 'farewell'
  if (
    FEEDBACK.test(t) ||
    CASUAL_ACK.test(t) ||
    (t.length < 48 &&
