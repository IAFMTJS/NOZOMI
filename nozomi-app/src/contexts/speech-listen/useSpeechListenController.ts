...
export function useSpeechListenController(): SpeechListenApi {
  ...
  const syncPresenceAfterTurn = useCallback(() => {
    ...
    setSpeechState('idle')
    if (!isSpeechOutputActive() && orb !== 'speaking') {
      setOrbState('idle')
    }
  ...
  useEffect(() => {
    if (isListenSessionActive()) {
      voiceDebug('ui:lang-change-cancel', { lang: recognitionLang })
      cancelListening()
      releaseSharedMicrophone()
  ...
  const handleStateChange = useCallback(
    (state: SpeechState) => {
      ...
      if (state === 'idle') finishingRef.current = false
    },
    [setOrbState, setSpeechState],
  )
  ...
  const beginListening = useCallback(() => {
    ...
    if (processingRef.current || finishingRef.current) {
      voiceDebugWarn('ui:begin-blocked', {
        reason: processingRef.current ? 'turn-processing' : 'finishing',
      })
      return false
    }
    ...
    if (isSpeechOutputActive()) stopSpeaking()
  ...
  const finishRecording = useCallback(() => {
    ...
    const waitForTranscript = () => {
      if (processingRef.current) return
      if (useUiStore.getState().speechState === 'error') return
